...
def addSource(line):
	fields = line.split("\t")
	sample = fields[6].split("/")[0]
	info = fields[20]
	sname = re.findall("SNAME=[^;]*;", info)[0]
	if sample in sname:
		fields[20] = "SOURCE=Callers;" + fields[20]
	else:
		fields[20] = "SOURCE=Regenotyping;" + fields[20]
	line = "\t".join(fields)	
	return line
	...
	with open(bedpe, "r") as f:
		for line in f:
			if "#" in line: #None are expected
				...
				if ann is not None:
					fields[20] = fields[20].rstrip() + ";" + ann
				new_line = "\t".join(fields)
				new_line = addSource(new_line)
				print(new_line.rstrip())
