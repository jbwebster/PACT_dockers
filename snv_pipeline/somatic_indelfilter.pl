...
# $URL: $
#----------------------------------
# Modifications made by Jace Webster
# Date: Apr 8 2021
#----------------------------------
use strict;
use warnings;
...
#        if ( ($t[34] + $t[36])  == 0 && ($t[41] + $t[43])/($t[39] + $t[41] + $t[43]) >= $paras{'vaf'} && (($t[41] + $t[43])/($t[40] + $t[41] + $t[43]) >= $paras{'vaf'} ) ) {
        if ( ($t[41] + $t[43]) == 0 && ($t[34] + $t[36])/($t[32] + $t[34] + $t[36]) >= $paras{'vaf'} && (($t[34] + $t[36])/($t[33] + $t[34] + $t[36]) >= $paras{'vaf'} ) ) {
            #print; print "\n";
            # allow complex
...
$pindel2vcf_command = "$paras{'pindel2vcf'} -R $paras{'referencename'} -r $paras{'reference'} -p $nocomplex_output -d $paras{'referencedate'} -v $pindel2vcf_output -G";
